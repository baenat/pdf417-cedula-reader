...
export class VideoCaptureComponent {
  ...
  pInit: Promise<{ cameraView: CameraView; cameraEnhancer: CameraEnhancer; router: CaptureVisionRouter; }> | null = null;

  @ViewChild('uiContainer') uiContainer: ElementRef<HTMLDivElement> | null = null;
  @ViewChild('resultsContainer') resultsContainer: ElementRef<HTMLDivElement> | null = null;
  responseData = { format: '', text: '' };

  async init(): Promise<{ cameraView: CameraView; cameraEnhancer: CameraEnhancer; router: CaptureVisionRouter; }> {
    try {
